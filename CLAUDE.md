...
## 3つの新機能（v1.2）の要点
- **共有ICS**：公開レベル3段階（公開/多忙のみ/非公開）。書き出しは「自分用（全件）」と「共有用（公開＋多忙マスクのみ。非公開は出力しない）」の2モード。※ICSでは秘匿は保証されないため、共有用は非公開を出力しない／タイトル差し替えで担保。
- **分析**：カテゴリ/象限/タグ/文脈/週次トレンド。無駄の検出（Q3・Q4の時間消費＋削減候補リスト）。
- **業務改善サイン（無駄検出とは別軸）**：Q3（重要でない×緊急）過多を検知し、定型化・委任・発生源対策の候補を提示。
- **Q3閾値はユーザーが都度変更可能、かつ期間（季節）ごとに設定可能**（繁忙期は緩め、通常期は厳しめ）。固定値にしない。
- **見積もり精度**：タグ別の見積÷実績で癖を可視化し、補正係数へ。実績はワンタップ記録を最優先（面倒ならカレンダー実時間からの推定で簡易版）。
